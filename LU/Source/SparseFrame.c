...
int SparseFrame_metis ( struct matrix_info_struct *matrix_info )
{
    ...
    {
        Cp = matrix_info->CPCTp;
        Ci = matrix_info->CPCTi;
    }
...
int SparseFrame_perm ( struct matrix_info_struct *matrix_info )
{
    ...
    {
        Uworkspace = matrix_info->workspace + 4 * nrow * sizeof(Long);
        UTworkspace = matrix_info->workspace + 5 * nrow * sizeof(Long);
    }
    ...
    if ( !isSymmetric )
    {
        ...
        memset ( UTp, 0, ( nrow + 1 ) * sizeof(Long) );
    }
                ...
                else
                {
                    ...
                    if ( j >= i )
                    {
                        ...
                        if ( !isComplex )
                            ...
                            ( (Complex*) UTx ) [utp] = ( (Complex*) Cx ) [pold];
                    }
                }
            }
        }
...
int SparseFrame_analyze ( struct common_info_struct *common_info, struct matrix_info_struct *matrix_info )
{
    ...
    if ( !isSymmetric )
    {
        ...
        CPCTi = malloc ( nzCPCT * sizeof(Long) );
    ...
    if ( !isSymmetric )
    {
        ...
        matrix_info->UTp = malloc ( ( nrow + 1 ) * sizeof(Long) );
        matrix_info->UTi = malloc ( nzmax * sizeof(Long) );
        if ( !isComplex )
            matrix_info->UTx = malloc ( nzmax * sizeof(Float) );
        else
            matrix_info->UTx = malloc ( nzmax * sizeof(Complex) );
    }
...
int SparseFrame_cpuApply ( int isComplex, Long *SuperMap, Long *Super, Long *Lsip, Long *Lsi, Long *Lsxp, void *Lsx, Long *Head, Long *Next, Long *Lpos, Long *Map, Long *RelativeMap, Long s, Long nscol, Long nsrow, void *A, Long d, void *C )
{
    ...
    if (!isComplex)
        dgemm_ ( "N", "T", &dnm, &dn, &dk, one, (Float*) Lsx + Lsxp[d] + lpos, &dlda, (Float*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos, &dlda, zero, (Float*) C, &dldc );
    else
        zgemm_ ( "N", "T", &dnm, &dn, &dk, (Complex*) one, (Complex*) Lsx + Lsxp[d] + lpos, &dlda, (Complex*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos, &dlda, (Complex*) zero, (Complex*) C, &dldc );

    if ( dm > 0 )
    {
        if (!isComplex)
            dgemm_ ( "N", "T", &dm, &dn, &dk, one, (Float*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos_next, &dlda, (Float*) Lsx + Lsxp[d] + lpos, &dlda, zero, (Float*) C + ( dn + dm ), &dldc );
        else
            zgemm_ ( "N", "T", &dm, &dn, &dk, (Complex*) one, (Complex*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos_next, &dlda, (Complex*) Lsx + Lsxp[d] + lpos, &dlda, (Complex*) zero, (Complex*) C + ( dn + dm ), &dldc );
    }
                        ...
                        for ( Long d_index = 0; d_index < gpu_blas_count; d_index++ )
                        {
                            ...
                            if ( cudaError == cudaSuccess )
                            {
                                ...
                                if ( GPUSerial[d] == gpuIndex && NodeLocation[d] == NODE_LOCATION_GPU )
                                {
                                    ...
                                    {
                                        if (!isComplex)
                                            cublasDgemm ( gpu_info->d_cublasHandle[stream_index], CUBLAS_OP_N, CUBLAS_OP_T, dm, dn, dk, one, (Float*) d_R + ( ndrow - ndcol ) + dn, dlda, (Float*) d_R, dlda, zero, (Float*) d_C + ( dn + dm ), dldc );
                                        else
                                            cublasZgemm ( gpu_info->d_cublasHandle[stream_index], CUBLAS_OP_N, CUBLAS_OP_T, dm, dn, dk, (Complex*) one, (Complex*) d_R + ( ndrow - ndcol ) + dn, dlda, (Complex*) d_R, dlda, (Complex*) zero, (Complex*) d_C + ( dn + dm ), dldc );
                                    }
...
#pragma omp parallel for schedule(auto) num_threads(CP_NUM_THREAD) if(ndcol>=CP_THREAD_THRESHOLD)
                                    for ( Long dj = 0; dj < ndcol; dj++ )
                                    {
                                        for ( Long di = 0; di < ndrow - lpos; di++ )
                                        {
                                            if (!isComplex)
                                            {
                                                ( (Float*) h_B ) [ dj * dlda + di ] = ( Lsx + Lsxp[d] + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ];
                                                ( (Float*) h_B + ( dn + dm ) ) [ dj * dlda + di ] = ( Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ];
                                            }
                                            else
                                            {
                                                ( (Complex*) h_B ) [ dj * dlda + di ].x = ( (Complex*) Lsx + Lsxp[d] + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ].x;
                                                ( (Complex*) h_B ) [ dj * dlda + di ].y = ( (Complex*) Lsx + Lsxp[d] + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ].y;
                                                ( (Complex*) h_B + ( dn + dm ) ) [ dj * dlda + di ].x = ( (Complex*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ].x;
                                                ( (Complex*) h_B + ( dn + dm ) ) [ dj * dlda + di ].y = ( (Complex*) Lsx + Lsxp[d] + ( ndrow - ndcol ) + lpos ) [ dj * ( 2 * ndrow - ndcol ) + di ].y;
                                            }
                                        }
                                    }
                                    ...
                                    {
                                        if (!isComplex)
                                            cublasDgemm ( gpu_info->d_cublasHandle[stream_index], CUBLAS_OP_N, CUBLAS_OP_T, dm, dn, dk, one, (Float*) d_B + ( dn + dm ) + dn, dlda, (Float*) d_B, dlda, zero, (Float*) d_C + ( dn + dm ), dldc );
                                        else
...
#pragma omp critical (HeadNext)
                        ...
                        deviceSum ( isComplex, d_A, d_A_reserve, d_A_, nscol, 2 * nsrow - nscol, slda, gpu_info->s_cudaStream );
...
#pragma omp parallel for schedule(auto) num_threads(CP_NUM_THREAD) if(nscol>=CP_THREAD_THRESHOLD)
                        ...
                        {
                            for ( Long si = 0; si < 2 * nsrow - nscol; si++ )
                            {
                                if ( !isComplex )
                            ...
                            for ( sjl = 0; sjl < nscol; sjl += sjblock )
                            {
                                ...
                                if ( sjl > 0 )
                                {
                                    ...
                                    if (!isComplex)
                                        ...
                                        cublasZgemm ( gpu_info->s_cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, nsrow - sjl, sjr - sjl, sjl, (Complex*) minus_one, (Complex*) d_A + sjl, slda, (Complex*) d_A + sjl * slda, slda, (Complex*) one, (Complex*) d_A + sjl * slda + sjl, slda );

                                    if ( sjr < nsrow )
                                    {
                                        if (!isComplex)
                                        {
                                            cublasDgemm ( gpu_info->s_cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, sjr - sjl, nscol - sjr, sjl, minus_one, (Float*) d_A + sjl, slda, (Float*) d_A + sjr * slda, slda, one, (Float*) d_A + sjr * slda + sjl, slda );
                                            cublasDgemm ( gpu_info->s_cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T, nsrow - nscol, sjr - sjl, sjl, minus_one, (Float*) d_A + ( nsrow - nscol ) + nscol, slda, (Float*) d_A + sjl, slda, one, (Float*) d_A + ( nsrow - nscol ) + sjl * slda + nscol, slda );
                                        }
                                        else
                                        {
                                            cublasZgemm ( gpu_info->s_cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, sjr - sjl, nscol - sjr, sjl, (Complex*) minus_one, (Complex*) d_A + sjl, slda, (Complex*) d_A + sjr * slda, slda, (Complex*) one, (Complex*) d_A + sjr * slda + sjl, slda );
                                            cublasZgemm ( gpu_info->s_cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T, nsrow - nscol, sjr - sjl, sjl, (Complex*) minus_one, (Complex*) d_A + ( nsrow - nscol ) + nscol, slda, (Complex*) d_A + sjl, slda, (Complex*) one, (Complex*) d_A + ( nsrow - nscol ) + sjl * slda + nscol, slda );
                                        }
                                    }
                                }
                                ...
                                if ( sjl > 0 )
                                {
                                    ...
                                    if ( !isComplex )
                                        cudaMemcpy2DAsync ( (Float*) h_A + sjl_last * slda, slda * sizeof(Float), (Float*) d_A + sjl_last * slda, slda * sizeof(Float), ( 2 * nsrow - nscol ) * sizeof(Float), sjblock, cudaMemcpyDeviceToHost, gpu_info->s_cudaStream_copyback );
                                    else
                                        cudaMemcpy2DAsync ( (Complex*) h_A + sjl_last * slda, slda * sizeof(Complex), (Complex*) d_A + sjl_last * slda, slda * sizeof(Complex), ( 2 * nsrow - nscol ) * sizeof(Complex), sjblock, cudaMemcpyDeviceToHost, gpu_info->s_cudaStream_copyback );
...
#pragma omp parallel for schedule(auto) num_threads(CP_NUM_THREAD) if(sjblock>=CP_THREAD_THRESHOLD)
                            ...
                            if ( !isComplex )
                                cudaMemcpy2DAsync ( (Float*) h_A + sjl_last * slda, slda * sizeof(Float), (Float*) d_A + sjl_last * slda, slda * sizeof(Float), ( 2 * nsrow - nscol ) * sizeof(Float), ( sjr_last - sjl_last ), cudaMemcpyDeviceToHost, gpu_info->s_cudaStream );
                            else
                                cudaMemcpy2DAsync ( (Complex*) h_A + sjl_last * slda, slda * sizeof(Complex), (Complex*) d_A + sjl_last * slda, slda * sizeof(Complex), ( 2 * nsrow - nscol ) * sizeof(Complex), ( sjr_last - sjl_last ), cudaMemcpyDeviceToHost, gpu_info->s_cudaStream );
...
int SparseFrame_solve_supernodal ( struct matrix_info_struct *matrix_info )
{
    ...
    for ( Long s = 0; s < nsuper; s++ )
    {
        ...
        for ( Long sj = 0; sj < nscol; sj++ )
        {
            ...
            for ( Long si = sj + 1; si < nsrow; si++ )
            {
                ...
                if ( !isComplex )
                    Xx[i] -= ( Lsx [ Lsxp[s] + sj * slda + si ] * Xx[j] );
                else
                {
                    // TODO
                }
    ...
    for ( Long s = nsuper - 1; s >= 0; s-- )
    {
        ...
        for ( Long sj = nscol - 1; sj >= 0; sj-- )
        {
            ...
            for ( Long si = sj + 1; si < nscol; si++ )
            {
                Long i = Lsi [ Lsip[s] + si ];

                if ( !isComplex )
                    Xx[j] -= ( Lsx [ Lsxp[s] + si * slda + sj ] * Xx[i] );
                else
                {
                    // TODO
                }
            ...
            for ( Long si = nscol; si < nsrow; si++ )
            {
                ...
                if ( !isComplex )
                    Xx[j] -= ( Lsx [ Lsxp[s] + ( nsrow - nscol ) + sj * slda + si ] * Xx[i] );
                else
                {
                    // TODO
                }
...
int SparseFrame_cleanup_matrix ( struct matrix_info_struct *matrix_info )
{
    ...
    if ( matrix_info->UTp != NULL ) free ( matrix_info->UTp );

...
int SparseFrame_read_matrix ( struct matrix_info_struct *matrix_info )
{
    ...
    matrix_info->workSize
        = MAX (
                ...
                11 * matrix_info->nrow * sizeof(Long) + 3 * matrix_info->nrow * sizeof(size_t)
              );
...
int SparseFrame_analyze_supernodal ( struct common_info_struct *common_info, struct matrix_info_struct *matrix_info )
{
    ...
    devSlotSize = ( common_info->minDevMemSize - nzmax * sizeof(Long) ) / 6;
...
int SparseFrame_factorize_supernodal ( struct common_info_struct *common_info, struct gpu_info_struct *gpu_info_list, struct matrix_info_struct *matrix_info )
{
    ...
    ST_Pointer_Tail = workspace + 6 * nsuper;
    Nschild = workspace + 7 * nsuper;
    Nstchild = workspace + 8 * nsuper;
    nodeState = (enum NodeState*) ( workspace + 9 * nsuper );
    ST_State = (enum NodeState*) ( workspace + 10 * nsuper );
    Aoffset = (size_t*) ( workspace + 11 * nsuper );
                                ...
                                for ( dpt = ST_Pointer[dt]; dpt < ST_Pointer_Tail[dt]; dpt++ )
                                {
                                    ...
                                    if ( ST_Map [ SuperMap [ Lsi [ Lsip[d] + lpos ] ] ] == st )
                                    {
                                        ...
                                        while ( lpos_0 < lpos_next )
                                        {
                                            ...
                                            if ( !isComplex )
                                            {
                                                d_C = gpu_info->devMem + 4 * devSlotSize + ( 1 - c_index ) * devSlotSize + Coffset[d] + ( ( lpos_0 - lpos ) * ( ndrow - lpos ) + ( lpos_0 - lpos ) ) * sizeof(Float);
                                            }
                                            else
                                            {
                                                d_C = gpu_info->devMem + 4 * devSlotSize + ( 1 - c_index ) * devSlotSize + Coffset[d] + ( ( lpos_0 - lpos ) * ( ndrow - lpos ) + ( lpos_0 - lpos ) ) * sizeof(Complex);
                                            }
                                        ...
                                        if ( dm > 0 )
                                        {
                                            if (!isComplex)
                                                cublasDgemm ( gpu_info->d_cublasHandle[slot_index], CUBLAS_OP_N, CUBLAS_OP_T,
                                                        dm, dn, dk, one, d_B + Aoffset[d] + dn * sizeof(Float), dlda, d_B + Aoffset[d], dlda, zero, d_C + dCsize + dn * sizeof(Float), dldc );
                                            else
...
#pragma omp parallel for private(di) num_threads(CP_NUM_THREAD) if(ndcol>CP_THREAD_THRESHOLD)
                                        for ( dj = 0; dj < ndcol; dj++ )
                                        {
                                            ...
                                            {
                                                if (!isComplex)
                                                    ( (Float*) ( h_B + Aoffset[d] ) ) [ dj * ( ndrow - lpos ) + di ] = ( Lsx + Lsxp[d] + lpos ) [ dj * ndrow + di ];
                                                else
                                                {
                                                    ( (Complex*) ( h_B + Aoffset[d] ) ) [ dj * ( ndrow - lpos ) + di ].x = ( (Complex*) Lsx + Lsxp[d] + lpos ) [ dj * ndrow + di ].x;
                                                    ( (Complex*) ( h_B + Aoffset[d] ) ) [ dj * ( ndrow - lpos ) + di ].y = ( (Complex*) Lsx + Lsxp[d] + lpos ) [ dj * ndrow + di ].y;
                                                }
                                            }
                                        }

                                        if ( !isComplex )
                                            cudaMemcpyAsync ( d_B + Aoffset[d], h_B + Aoffset[d], ndcol * ( ndrow - lpos ) * sizeof(Float), cudaMemcpyHostToDevice, gpu_info->d_cudaStream[slot_index] );
                                        else
                                            cudaMemcpyAsync ( d_B + Aoffset[d], h_B + Aoffset[d], ndcol * ( ndrow - lpos ) * sizeof(Complex), cudaMemcpyHostToDevice, gpu_info->d_cudaStream[slot_index] );
                                    }
                                }
